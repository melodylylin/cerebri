...
static void rdd2_manual_run(void* p0, void* p1, void* p2)
{
    ...
    while (atomic_get(&ctx->running)) {
        ...
        if (ctx->status.mode == synapse_msgs_Status_Mode_MODE_MANUAL) {
            ...
            ctx->velocity_sp.x = 2 * (double)ctx->joy.axes[JOY_AXES_ROLL];
            ctx->velocity_sp.y = 2 * (double)ctx->joy.axes[JOY_AXES_PITCH];
            ctx->velocity_sp.z = 2 * (double)ctx->joy.axes[JOY_AXES_THRUST];
            zros_pub_update(&ctx->pub_velocity_sp);

            ctx->orientation_sp.z = -20 * deg2rad * (double)ctx->joy.axes[JOY_AXES_YAW];
